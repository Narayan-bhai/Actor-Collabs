...
export default function App() {
  ...
  const nodeRadius = (node, highlighted = false) => {
    const base = Math.max(4, Math.min(14, 3 + Math.sqrt(node.degree || 1) * 1.6));
  ...
  const linkCanvasObject = useCallback((link, ctx) => {
    ...
    const genreRelevant = genre === null || srcGenre === genre || tgtGenre === genre;
    ...
    let alpha;
    if (genre !== null && !genreRelevant) alpha = 0.04;  // fade unrelated links
            ...
            <div className="box" id="legend-box">
              ...
              {selectedGenre && (
                <p className="genre-filter-hint">
                  Showing: <strong style={{ color: genreColor(selectedGenre) }}>{selectedGenre}</strong>
                </p>
              )}
              <div id="legend-grid">
                {visibleGenres.map(g => (

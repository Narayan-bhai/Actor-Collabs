...
function App() {
  ...
  const defaultActors = ["Alice", "Bob", "Charlie", "Diana"];
  ...
  useEffect(() => {
    fetch("http://localhost:5000/matrix")
      ...
      .then(data => {
        ...
        const allActors = Array.from(new Set([...defaultActors, ...actors]));
        ...
        for (let i = 0; i < matrix.length; i++) {
          for (let j = i + 1; j < matrix.length; j++) {
            if (matrix[i][j] > 0) {
              links.push({ source: allActors[i], target: allActors[j], value: matrix[i][j] });
              degrees[i] += 1;
              degrees[j] += 1;
            }
          }
        ...
        // Stats
        const nodeStats = allActors.map((name, i) => ({ name, collaborators: degrees[i] || 0 }));
        nodeStats.sort((a, b) => b.collaborators - a.collaborators);
        setStats({ totalActors: allActors.length, totalLinks: links.length, nodeStats });
  ...
  graphData.nodes.forEach(n => {
    if (!clusterColors[n.cluster]) clusterColors[n.cluster] = distinctColors[n.cluster % distinctColors.length];
  });
  ...
  const highlightLargestCluster = () => {
    const clusterCounts = {};
    Object.values(clusters).forEach(c => clusterCounts[c] = (clusterCounts[c] || 0) + 1);
    const largestClusterId = Number(Object.entries(clusterCounts).sort((a, b) => b[1] - a[1])[0][0]);
    const clusterNodes = graphData.nodes.filter(n => n.cluster === largestClusterId).map(n => n.id);
  ...
  const dijkstra = (fromInput, toInput) => {
    ...
    while (Q.size) {
      let u = null;
      Q.forEach(node => { if (u === null || dist[node] < dist[u]) u = node; });
      if (u === to) break;
      Q.delete(u);
    ...
    while (u !== undefined) {
      pathNodes.unshift(u);
      if (prev[u] !== undefined) pathEdges.add(`${prev[u]}___${u}`);
      u = prev[u];
    }
  ...
  const runDijkstra = () => {
    const result = dijkstra(dijkstraInput.from, dijkstraInput.to);
    if (result) {
    ...
    <div id="container">
      <div id="sidebar">
        ...
        {stats && <>
          ...
          <h3>Functions</h3>
          <button onClick={highlightTopCollaborators}>Top Collaborators</button>
          <button onClick={highlightLargestCluster}>Largest Cluster</button>
          ...
          <input
            ...
            onChange={e => setDijkstraInput({ ...dijkstraInput, from: e.target.value })}
          />
          <input
            placeholder="To Actor"
            value={dijkstraInput.to}
            onChange={e => setDijkstraInput({ ...dijkstraInput, to: e.target.value })}
          />
          <button onClick={runDijkstra}>Compute Path</button>
        </>}
      </div>
      ...
      <div id="graph-container">
        <ForceGraph2D
          ...
          linkWidth={link => {
            ...
            return highlightEdges.has(`${src}___${tgt}`) || highlightEdges.has(`${tgt}___${src}`) ? 3 : Math.sqrt(link.value || 1);
          }}
          linkColor={link => {
            const src = typeof link.source === "object" ? link.source.id : link.source;
            const tgt = typeof link.target === "object" ? link.target.id : link.target;
            const srcCluster = clusters[src] ?? 0;
            return highlightEdges.has(`${src}___${tgt}`) || highlightEdges.has(`${tgt}___${src}`) ? "#ffff00" : clusterColors[srcCluster] || "#aaa";
          }}
          ...
          nodeCanvasObject={(node, ctx, globalScale) => {
            ...
            const baseColor = clusterColors[clusters[node.id]] || "#888";
            ctx.fillStyle = highlightNodes.has(node.id) ? "#ff9900" : baseColor;
            ctx.beginPath();
